...
fn main(){
	...
	*/
	...
	let initial_state = State{
		cur_floor: 0,
		floors: [
			FloorState{ gens: 1<<(Elem::Strontium as u8)|1<<(Elem::Plutonium as u8), chips: 1<<(Elem::Strontium as u8)|1<<(Elem::Plutonium as u8) },
	...
	'mainloop: while queue.len() > 0 {
		...
		println!("foo {}; {} {}", dist, visited_states.len(), queue.len());
	}
}
